...
class Network(object):       # clase del objeto con argumneto su herencia, es decir,
    ...
    def SG(self, training_data, epochs, mini_batch_size, eta,
            test_data=None):
#Aquí se definio el optimizador Stochastic Gradient Descent, con trainig data como la lista de Tuplas "(x,y)"
    ...
    """""
    def SG_momentum(self, training_data, epochs, mini_batch_size, eta, momentum, test_data=None):
        training_data = list(training_data)
        if test_data: n_test = len(test_data) #trasformar los datos de entrenamiento en una lista, o tupla
        n = len(training_data)  #El número de datos tenemos para entrenar
    ...
    def update_mini_batch_SGD_momentum(self, mini_batch, eta, momentum ): #para cada mini_batch se aplica un único paso de descenso 
                                             #del gradiente.

        nabla_b = [np.zeros(b.shape) for b in self.biases]#una vez calculados los gradientes se guardan
        nabla_w = [np.zeros(w.shape) for w in self.weights]
        vdw = [np.zeros(b.shape) for b in self.biases]#una vez calculados los gradientes se guardan
        vdb = [np.zeros(w.shape) for w in self.weights]
        ...
        for x, y in mini_batch: # Aquí x es la entrada de la red, y y es el dato real que queremos que nos 
            ...
            #vdb = [dnb-eta*(momentum) for nvdb, dnb in zip(vdb, nabla_b)] 
    ...
    # Inicializa la velocidad de los pesos y los sesgos
    ...
    for j in range(epochs):
        ...
        for mini_batch in mini_batches:
            self.update_mini_batch_momentum(mini_batch, eta, momentum, v_w, v_b)
        if test_data:
            print("Epoch {} : {} / {}".format(j,self.evaluate(test_data),n_test))
        else:
            print("Epoch {} complete".format(j))

    def update_mini_batch_momentum(self, mini_batch, eta, momentum, v_w, v_b):
    ...
    # Actualiza la velocidad de los pesos y los sesgos
    v_w = [momentum * vw - (eta / len(mini_batch)) * nw
           for vw, nw in zip(v_w, nabla_w)]
    v_b = [momentum * vb - (eta / len(mini_batch)) * nb
           for vb, nb in zip(v_b, nabla_b)]

    # Actualiza los pesos y los sesgos utilizando la velocidad calculada
    self.weights = [w + vw for w, vw in zip(self.weights, v_w)]
    self.biases = [b + vb for b, vb in zip(self.biases, v_b)]


    """""
    def SGM(self, training_data, epochs, mini_batch_size, eta, momentum,
                  test_data=None):
    training_data = list(training_data)
    n = len(training_data)
