...
class Network(object):       # clase del objeto con argumneto su herencia, es decir,
    ...
    def feedforward(self, a): #Se define feedforward, para evaluar la red neuronal
        ...
        return a
    def SG_momentum(self, training_data, epochs, mini_batch_size, eta, momentum,
            test_data=None):
#Aquí se definio el optimizador Stochastic Gradient Descent, con trainig data como la lista de Tuplas "(x,y)"
        training_data = list(training_data)
        #if test_data: 
        # n_test = len(test_data) #trasformar los datos de entrenamiento en una lista, o tupla
    ...
    def update_mini_batch_momentum(self, mini_batch, eta, momentum): #para cada mini_batch se aplica un único paso de descenso 
                                             #del gradiente.

        nabla_b = [np.zeros(b.shape) for b in self.biases]#una vez calculados los gradientes se guardan
        nabla_w = [np.zeros(w.shape) for w in self.weights]
        v_b = [np.zeros(b.shape) for b in self.biases]#una vez calculados los gradientes se guardan
        v_w = [np.zeros(w.shape) for w in self.weights]
        for x, y in mini_batch: 
            ...
            v_w = [ momentum * vw +(eta /len(mini_batch)) * nw 
                   for vw, nw in zip(self.weights, nabla_w) ]
            v_b = [ momentum * vb +(eta /len(mini_batch)) * nb 
                   for vb, nb in zip(self.weights, nabla_b) ]
